...
- **Top-level only**: File discovery is limited to the top level of the source folder. Subdirectories are not recursed.
...
- **Interactive prompts**: Agent conversion from Claude Code to OpenCode requires interactive prompts unless `--model` and `--steps` are provided. This makes non-interactive batch conversion impossible without CLI flags.
...
- [ ] Recursive file discovery with configurable depth
...
- [ ] Validation mode: check that converted agents have valid `model` and `steps` values before writing
- [ ] Dry-run mode: preview changes without writing files
- [ ] Configurable field mappings via YAML config file
- [ ] Automatic `license` and `compatibility` derivation for skills (e.g., from git metadata)
